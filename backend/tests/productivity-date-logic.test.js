...
test('generateReport: task without Ngày Làm is counted but not scored', async () => {
    ...
    // Find the user row (could be in validData or unknownUsers depending on SENIORITY_MAPPING)
    const allRows = [...result.validData, ...(result.unknownUsers || [])];
    const userRow = result.validData.find(r => r.fullName === 'TestUser')
        || result.unknownUsers?.find(r => r.name === 'TestUser');

    // taskCount should include the no-date task
    if (userRow && userRow.taskCount !== undefined) {
        assert.equal(userRow.taskCount, 2, 'taskCount should include both tasks (1 in-range + 1 no-date)');
    }
